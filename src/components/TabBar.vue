<template>
  <div id="head">
    <menu class="menu" ref="menu">
      <bar-item v-for="[index, item] in barItemLists.entries()"
                :color="item.color"
                :active="item.active"
                :key="index"
                :ref="index"
                :path="item.path"
                @accepted="clickItem(index)"
      >
      </bar-item>
      <div class="menu__border" ref="menu_border"></div>
    </menu>
...
const barItemData = function (color, active, path) {
  this.color = color;
  this.active = active;
  this.path = path;
}
...
export default {
  ...
  data() {
    return {
      barItemLists: [
        new barItemData("#ffb457", true, [
          "M3.8,6.6h16.4",
          "M20.2,12.1H3.8",
          "M3.8,17.5h16.4"
        ]),
        new barItemData("#ff96bd", false, [
  ...
  methods: {
    clickItem(index) {
      let activeItem = this.$refs[index][0].$refs.button;
      this.barItemLists[this.last].active = false;
      this.barItemLists[index].active = true;
      this.last = index;
      this.offsetMenuBorder(activeItem);
    },
    offsetMenuBorder(activeItem) {
      const offsetActiveItem = activeItem.getBoundingClientRect();
      const left = Math.floor(offsetActiveItem.left - this.menu.offsetLeft - (this.menuBorder.offsetWidth - offsetActiveItem.width) / 2) + "px";
